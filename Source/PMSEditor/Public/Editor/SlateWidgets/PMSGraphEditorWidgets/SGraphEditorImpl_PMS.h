...
struct FAlignmentData_PMS
{
	FAlignmentData_PMS(UEdGraphNode* InNode, int32& InTargetProperty, float InTargetOffset)
...
enum class EAlignType_PMS : uint8
{
	Minimum, Middle, Maximum
};

/** Helper class for aligning nodes */
struct FAlignmentHelper_PMS
{
	/** Construct from a graph editor, an orientation, and an alignment type */
	FAlignmentHelper_PMS(TSharedRef<SGraphEditor> InGraphEditor, EOrientation InOrientation, EAlignType_PMS InAlignType)
		: GraphEditor(MoveTemp(InGraphEditor))
	{
		...
		AlignmentData.Sort([](const FAlignmentData_PMS& A, const FAlignmentData_PMS& B) {
		...
		if (AlignmentData.Num() > 1)
		{
			...
			if (Graph)
			{
				...
				{ 
					...
					for (FAlignmentData_PMS& Entry : AlignmentData)
					{
...
private:
	...
	FAlignmentData_PMS GetAlignmentDataForNode(UEdGraphNode* Node);
	...
	float DetermineAlignmentTarget()
	{
		...
		if (AlignType == EAlignType_PMS::Minimum)
		{
			float Target = TNumericLimits<float>::Max();
			for (const FAlignmentData_PMS& Entry : AlignmentData)
			{
				Target = FMath::Min(Target, Entry.GetTarget());
			}
			return Target;
		}
		else if (AlignType == EAlignType_PMS::Maximum)
		{
	...
	EAlignType_PMS AlignType;
	/** The cardinal node that all other nodes should align to (possibly null) */
	UEdGraphNode* CardinalNode;
	/** Generated alignment data */
	TArray<FAlignmentData_PMS> AlignmentData;
};

class SGraphEditorImpl_PMS : public SGraphEditor_PMS
{
public:
	SLATE_BEGIN_ARGS( SGraphEditorImpl_PMS )
