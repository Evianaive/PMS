#include "SGraphEditorImpl_PMS.h"
...
#define LOCTEXT_NAMESPACE "GraphEditorModule_PMS"
...
/////////////////////////////////////////////////////
// FAlignmentHelper_PMS

FAlignmentData_PMS FAlignmentHelper_PMS::GetAlignmentDataForNode(UEdGraphNode* Node)
{
	...
	switch (AlignType)
	{
	case EAlignType_PMS::Minimum:	PropertyOffset = 0.f;			break;
	case EAlignType_PMS::Middle:	PropertyOffset = NodeSize * .5f;	break;
	case EAlignType_PMS::Maximum:	PropertyOffset = NodeSize;		break;
	}
	int32* Property = Orientation == Orient_Horizontal ? &Node->NodePosX : &Node->NodePosY;
	return FAlignmentData_PMS(Node, *Property, PropertyOffset);
}

/////////////////////////////////////////////////////
// SGraphEditorImpl_PMS

FVector2D SGraphEditorImpl_PMS::GetPasteLocation() const
{
	return GraphPanel->GetPastePosition();
}

bool SGraphEditorImpl_PMS::IsNodeTitleVisible( const UEdGraphNode* Node, bool bEnsureVisible )
{
	return GraphPanel->IsNodeTitleVisible(Node, bEnsureVisible);
}

void SGraphEditorImpl_PMS::JumpToNode( const UEdGraphNode* JumpToMe, bool bRequestRename, bool bSelectNode )
{
...
void SGraphEditorImpl_PMS::JumpToPin( const UEdGraphPin* JumpToMe )
{
...
bool SGraphEditorImpl_PMS::SupportsKeyboardFocus() const
{
	return true;
}

FReply SGraphEditorImpl_PMS::OnFocusReceived( const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent )
{
	OnFocused.ExecuteIfBound(SharedThis(this));
	return FReply::Handled();
}

FReply SGraphEditorImpl_PMS::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
FReply SGraphEditorImpl_PMS::OnMouseMove( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
FReply SGraphEditorImpl_PMS::OnKeyDown( const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent )
{
...
void SGraphEditorImpl_PMS::NotifyGraphChanged()
{
	FEdGraphEditAction DefaultAction;
	OnGraphChanged(DefaultAction);
}

void SGraphEditorImpl_PMS::OnGraphChanged(const FEdGraphEditAction& InAction)
{
	if ( !bIsActiveTimerRegistered )
	{
			...
			(!bWasAddAction && !bWasSelectAction && !bWasRemoveAction) )
		{
			...
			RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateSP(this, &SGraphEditorImpl_PMS::TriggerRefresh));
		}
...
void SGraphEditorImpl_PMS::GetPinContextMenuActionsForSchema(UToolMenu* InMenu) const
{
	...
	// Select Output/Input Nodes
	{
		...
		SelectConnectedNodesAction.ExecuteAction = FToolMenuExecuteAction::CreateSP(this, &SGraphEditorImpl_PMS::ExecuteSelectConnectedNodesFromPin);
		SelectConnectedNodesAction.IsActionVisibleDelegate = FToolMenuIsActionButtonVisible::CreateSP(this, &SGraphEditorImpl_PMS::IsSelectConnectedNodesFromPinVisible, EEdGraphPinDirection::EGPD_Output);
		...
		SelectConnectedNodesAction.IsActionVisibleDelegate = FToolMenuIsActionButtonVisible::CreateSP(this, &SGraphEditorImpl_PMS::IsSelectConnectedNodesFromPinVisible, EEdGraphPinDirection::EGPD_Input);
	...
	// Break all pin links
	{
		...
		BreakPinLinksAction.ExecuteAction = FToolMenuExecuteAction::CreateSP(this, &SGraphEditorImpl_PMS::ExecuteBreakPinLinks);
		BreakPinLinksAction.IsActionVisibleDelegate = FToolMenuIsActionButtonVisible::CreateSP(this, &SGraphEditorImpl_PMS::IsBreakPinLinksVisible);
	...
	// Break Specific Links
	{
		...
		BreakLinksMenuVisibility.IsActionVisibleDelegate = FToolMenuIsActionButtonVisible::CreateSP(this, &SGraphEditorImpl_PMS::IsBreakPinLinksVisible);
	...
	PinActionSubMenuVisibiliity.IsActionVisibleDelegate = FToolMenuIsActionButtonVisible::CreateSP(this, &SGraphEditorImpl_PMS::HasAnyLinkedPins);
		...
		Section.AddSubMenu("StraightenPinConnection",
			...
			FNewToolMenuDelegate::CreateLambda([this, GetMenuEntryForPin](UToolMenu* InToolMenu)
			{
				...
				// Add straighten all connected pins
				{
					...
					StraightenConnectionsAction.IsActionVisibleDelegate = FToolMenuIsActionButtonVisible::CreateSP(this, &SGraphEditorImpl_PMS::HasAnyLinkedPins);
...
void SGraphEditorImpl_PMS::ExecuteBreakPinLinks(const FToolMenuContext& InContext) const
{
...
bool SGraphEditorImpl_PMS::IsBreakPinLinksVisible(const FToolMenuContext& InContext) const
{
	UGraphNodeContextMenuContext* NodeContext = InContext.FindContext<UGraphNodeContextMenuContext>();
	if (NodeContext && NodeContext->Pin)
	{
...
bool SGraphEditorImpl_PMS::HasAnyLinkedPins(const FToolMenuContext& InContext) const
{
	UGraphNodeContextMenuContext* NodeContext = InContext.FindContext<UGraphNodeContextMenuContext>();
	if (NodeContext && NodeContext->Pin)
	{
...
void SGraphEditorImpl_PMS::ExecuteSelectConnectedNodesFromPin(const FToolMenuContext& InContext) const
{
	UGraphNodeContextMenuContext* NodeContext = InContext.FindContext<UGraphNodeContextMenuContext>();
	if (NodeContext && NodeContext->Pin)
	{
...
void SGraphEditorImpl_PMS::SelectAllNodesInDirection(const UEdGraphPin* InGraphPin) const
{
	...
	{
		const_cast<SGraphEditorImpl_PMS*>(this)->SetNodeSelection(const_cast<UEdGraphNode*>(Node), true);
	}
}

bool SGraphEditorImpl_PMS::IsSelectConnectedNodesFromPinVisible(const FToolMenuContext& InContext, EEdGraphPinDirection DirectionToSelect) const
{
	UGraphNodeContextMenuContext* NodeContext = InContext.FindContext<UGraphNodeContextMenuContext>();
	if (NodeContext && NodeContext->Pin)
	{
...
//void SGraphEditorImpl_PMS::GraphEd_OnPanelUpdated()
//{
//	
//}

const TSet<class UObject*>& SGraphEditorImpl_PMS::GetSelectedNodes() const
{
	return GraphPanel->SelectionManager.GetSelectedNodes();
}

void SGraphEditorImpl_PMS::ClearSelectionSet()
{
	GraphPanel->SelectionManager.ClearSelectionSet();
}

void SGraphEditorImpl_PMS::SetNodeSelection(UEdGraphNode* Node, bool bSelect)
{
	GraphPanel->SelectionManager.SetNodeSelection(Node, bSelect);
}

void SGraphEditorImpl_PMS::SelectAllNodes()
{
...
UEdGraphPin* SGraphEditorImpl_PMS::GetGraphPinForMenu()
{
	return GraphPinForMenu.Get();
}

UEdGraphNode* SGraphEditorImpl_PMS::GetGraphNodeForMenu()
{
	return GraphNodeForMenu.Get();
}

void SGraphEditorImpl_PMS::ZoomToFit(bool bOnlySelection)
{
	GraphPanel->ZoomToFit(bOnlySelection);
}
bool SGraphEditorImpl_PMS::GetBoundsForSelectedNodes( class FSlateRect& Rect, float Padding )
{
	return GraphPanel->GetBoundsForSelectedNodes(Rect, Padding);
}

bool SGraphEditorImpl_PMS::GetBoundsForNode( const UEdGraphNode* InNode, class FSlateRect& Rect, float Padding) const
{
...
void SGraphEditorImpl_PMS::StraightenConnections()
{
...
void SGraphEditorImpl_PMS::StraightenConnections(UEdGraphPin* SourcePin, UEdGraphPin* PinToAlign) const
{
	const FScopedTransaction Transaction(FGraphEditorCommands::Get().StraightenConnections->GetLabel());
	GraphPanel->StraightenConnections(SourcePin, PinToAlign);
}

void SGraphEditorImpl_PMS::RefreshNode(UEdGraphNode& Node)
{
	GraphPanel->RefreshNode(Node);
}

void SGraphEditorImpl_PMS::Construct( const FArguments& InArgs )
{
	...
	// Tell GraphEditor how to handle all the known commands
	{
		Commands->MapAction( FGraphEditorCommands::Get().ReconstructNodes,
			FExecuteAction::CreateSP( this, &SGraphEditorImpl_PMS::ReconstructNodes ),
			FCanExecuteAction::CreateSP( this, &SGraphEditorImpl_PMS::CanReconstructNodes )
		);

		Commands->MapAction( FGraphEditorCommands::Get().BreakNodeLinks,
			FExecuteAction::CreateSP( this, &SGraphEditorImpl_PMS::BreakNodeLinks ),
			FCanExecuteAction::CreateSP( this, &SGraphEditorImpl_PMS::CanBreakNodeLinks )
		);

		Commands->MapAction(FGraphEditorCommands::Get().SummonCreateNodeMenu,
			FExecuteAction::CreateSP(this, &SGraphEditorImpl_PMS::SummonCreateNodeMenu),
			FCanExecuteAction::CreateSP(this, &SGraphEditorImpl_PMS::CanSummonCreateNodeMenu)
		);

		Commands->MapAction(FGraphEditorCommands::Get().StraightenConnections,
			FExecuteAction::CreateSP(this, &SGraphEditorImpl_PMS::StraightenConnections)
		);
	...
	[
		...
		[
			SAssignNew(GraphPanel, SPMSGraphPanel)
			.GraphObj( EdGraphObj )
			.GraphObjToDiff( InArgs._GraphToDiff)
			.OnGetContextMenuFor( this, &SGraphEditorImpl_PMS::GraphEd_OnGetContextMenuFor )
			.OnSelectionChanged( InArgs._GraphEvents.OnSelectionChanged )
			.OnNodeDoubleClicked( InArgs._GraphEvents.OnNodeDoubleClicked )
			.IsEditable( this, &SGraphEditorImpl_PMS::IsGraphEditable )
			.DisplayAsReadOnly( this, &SGraphEditorImpl_PMS::DisplayGraphAsReadOnly )
			...
			//.OnUpdateGraphPanel( this, &SGraphEditorImpl_PMS::GraphEd_OnPanelUpdated )
		...
		[
			...
			.Text( this, &SGraphEditorImpl_PMS::GetZoomText )
			.ColorAndOpacity( this, &SGraphEditorImpl_PMS::GetZoomTextColorAndOpacity )
		]
		...
		[
			...
			[
				...
				.BorderBackgroundColor(this, &SGraphEditorImpl_PMS::InstructionBorderColor)
				.HAlign(HAlign_Center)
				.ColorAndOpacity(this, &SGraphEditorImpl_PMS::InstructionTextTint)
				.Visibility(this, &SGraphEditorImpl_PMS::InstructionTextVisibility)
				[
					SNew(STextBlock)
					.TextStyle(FEditorStyle::Get(), "Graph.InstructionText")
					.Text(this, &SGraphEditorImpl_PMS::GetInstructionText)
				]
			]			
		]
		...
		[
			SNew(STextBlock)
			.Visibility(this, &SGraphEditorImpl_PMS::PIENotification)
		...
		[
			SNew(STextBlock)
			.Visibility(this, &SGraphEditorImpl_PMS::ReadOnlyVisibility)
			.TextStyle(FEditorStyle::Get(), "Graph.CornerText")
			.Text(ReadOnlyText)
		]
...
EVisibility SGraphEditorImpl_PMS::PIENotification( ) const
{
...
SGraphEditorImpl_PMS::~SGraphEditorImpl_PMS()
{
}

void SGraphEditorImpl_PMS::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
EActiveTimerReturnType SGraphEditorImpl_PMS::TriggerRefresh( double InCurrentTime, float InDeltaTime )
{
	GraphPanel->Update();
...
void SGraphEditorImpl_PMS::OnClosedActionMenu()
{
	GraphPanel->OnStopMakingConnection(/*bForceStop=*/ true);
}

void SGraphEditorImpl_PMS::AddContextMenuCommentSection(UToolMenu* InMenu)
{
...
FName SGraphEditorImpl_PMS::GetNodeParentContextMenuName(UClass* InClass)
{
...
FName SGraphEditorImpl_PMS::GetNodeContextMenuName(UClass* InClass)
{
	return FName(*(FString(TEXT("GraphEditor.GraphNodeContextMenu.")) + InClass->GetName()));
}

void SGraphEditorImpl_PMS::RegisterContextMenu(const UEdGraphSchema* Schema, FToolMenuContext& MenuContext) const
{
	...
	if (bDidRegisterGraphSchemaMenu)
	{
		...
		Menu->AddDynamicSection("EdGraphSchemaPinActions", FNewToolMenuDelegate::CreateLambda([](UToolMenu* InMenu)
			{
				...
				{
					if (TSharedPtr<SGraphEditorImpl_PMS> GraphEditor = StaticCastSharedPtr<SGraphEditorImpl_PMS>(FindGraphEditorForGraph(NodeContext->Graph)))
					{
		...
		Menu->AddDynamicSection("EdGraphSchema", FNewToolMenuDelegate::CreateStatic(&SGraphEditorImpl_PMS::AddContextMenuCommentSection));
	}
...
UToolMenu* SGraphEditorImpl_PMS::GenerateContextMenu(const UEdGraphSchema* Schema, FToolMenuContext& MenuContext) const
{
...
FActionMenuContent SGraphEditorImpl_PMS::GraphEd_OnGetContextMenuFor(const FGraphContextMenuArguments& SpawnInfo)
{
			...
			if (EdGraphObj->GetSchema() != NULL )
			{
				if(OnCreateActionMenu.IsBound())
				{
					Result = OnCreateActionMenu.Execute(
						...
						SGraphEditor::FActionMenuClosed::CreateSP(this, &SGraphEditorImpl_PMS::OnClosedActionMenu)
						);
				}
				else
				{
					TSharedRef<SGraphEditorActionMenu> Menu =	
						...
						.OnClosedCallback( SGraphEditor::FActionMenuClosed::CreateSP(this, &SGraphEditorImpl_PMS::OnClosedActionMenu)
						);
...
bool SGraphEditorImpl_PMS::CanReconstructNodes() const
{
	return IsGraphEditable() && (GraphPanel->SelectionManager.AreAnyNodesSelected());
}

bool SGraphEditorImpl_PMS::CanBreakNodeLinks() const
{
	return IsGraphEditable() && (GraphPanel->SelectionManager.AreAnyNodesSelected());
}

bool SGraphEditorImpl_PMS::CanSummonCreateNodeMenu() const
{
	return IsGraphEditable() && GraphPanel->IsHovered() && GetDefault<UGraphEditorSettings>()->bOpenCreateMenuOnBlankGraphAreas;
}

void SGraphEditorImpl_PMS::ReconstructNodes()
{
...
void SGraphEditorImpl_PMS::BreakNodeLinks()
{
...
void SGraphEditorImpl_PMS::SummonCreateNodeMenu()
{
	GraphPanel->SummonCreateNodeMenuFromUICommand(NumNodesAddedSinceLastPointerPosition);
}

FText SGraphEditorImpl_PMS::GetZoomText() const
{
	return GraphPanel->GetZoomText();
}

FSlateColor SGraphEditorImpl_PMS::GetZoomTextColorAndOpacity() const
{
	return GraphPanel->GetZoomTextColorAndOpacity();
}

bool SGraphEditorImpl_PMS::IsGraphEditable() const
{
	return (EdGraphObj != NULL) && IsEditable.Get();
}

bool SGraphEditorImpl_PMS::DisplayGraphAsReadOnly() const
{
	return (EdGraphObj != NULL) && DisplayAsReadOnly.Get();
}

bool SGraphEditorImpl_PMS::IsLocked() const
{
...
TSharedPtr<SWidget> SGraphEditorImpl_PMS::GetTitleBar() const
{
	return TitleBar;
}

void SGraphEditorImpl_PMS::SetViewLocation( const FVector2D& Location, float ZoomAmount, const FGuid& BookmarkId ) 
{
...
void SGraphEditorImpl_PMS::GetViewLocation( FVector2D& Location, float& ZoomAmount ) 
{
	if( GraphPanel.IsValid() &&  EdGraphObj && (!IsLocked() || !GraphPanel->HasDeferredObjectFocus()))
	{
...
void SGraphEditorImpl_PMS::GetViewBookmark( FGuid& BookmarkId )
{
...
void SGraphEditorImpl_PMS::LockToGraphEditor( TWeakPtr<SGraphEditor> Other ) 
{
...
void SGraphEditorImpl_PMS::UnlockFromGraphEditor( TWeakPtr<SGraphEditor> Other )
{
	check(Other.IsValid());
...
void SGraphEditorImpl_PMS::AddNotification( FNotificationInfo& Info, bool bSuccess )
{
...
void SGraphEditorImpl_PMS::FocusLockedEditorHere()
{
...
void SGraphEditorImpl_PMS::SetPinVisibility( SGraphEditor::EPinVisibility InVisibility ) 
{
	if( GraphPanel.IsValid())
	{
...
TSharedRef<FActiveTimerHandle> SGraphEditorImpl_PMS::RegisterActiveTimer(float TickPeriod, FWidgetActiveTimerDelegate TickFunction)
{
	return SWidget::RegisterActiveTimer(TickPeriod, TickFunction);
}

EVisibility SGraphEditorImpl_PMS::ReadOnlyVisibility() const
{
...
FText SGraphEditorImpl_PMS::GetInstructionText() const
{
...
EVisibility SGraphEditorImpl_PMS::InstructionTextVisibility() const
{
...
float SGraphEditorImpl_PMS::GetInstructionTextFade() const
{
...
FLinearColor SGraphEditorImpl_PMS::InstructionTextTint() const
{
	return FLinearColor(1.f, 1.f, 1.f, GetInstructionTextFade());
}

FSlateColor SGraphEditorImpl_PMS::InstructionBorderColor() const
{
...
void SGraphEditorImpl_PMS::CaptureKeyboard()
{
	FSlateApplication::Get().SetKeyboardFocus(GraphPanel);
}

void SGraphEditorImpl_PMS::SetNodeFactory(const TSharedRef<class FGraphNodeFactory>& NewNodeFactory)
{
	GraphPanel->SetNodeFactory(NewNodeFactory);
}


void SGraphEditorImpl_PMS::OnAlignTop()
{
	const FScopedTransaction Transaction(FGraphEditorCommands::Get().AlignNodesTop->GetLabel());

	FAlignmentHelper_PMS Helper(SharedThis(this), Orient_Vertical, EAlignType_PMS::Minimum);
	Helper.Align();
}

void SGraphEditorImpl_PMS::OnAlignMiddle()
{
	const FScopedTransaction Transaction(FGraphEditorCommands::Get().AlignNodesMiddle->GetLabel());

	FAlignmentHelper_PMS Helper(SharedThis(this), Orient_Vertical, EAlignType_PMS::Middle);
	Helper.Align();
}

void SGraphEditorImpl_PMS::OnAlignBottom()
{
	const FScopedTransaction Transaction(FGraphEditorCommands::Get().AlignNodesBottom->GetLabel());

	FAlignmentHelper_PMS Helper(SharedThis(this), Orient_Vertical, EAlignType_PMS::Maximum);
	Helper.Align();
}

void SGraphEditorImpl_PMS::OnAlignLeft()
{
	const FScopedTransaction Transaction(FGraphEditorCommands::Get().AlignNodesLeft->GetLabel());

	FAlignmentHelper_PMS Helper(SharedThis(this), Orient_Horizontal, EAlignType_PMS::Minimum);
	Helper.Align();
}

void SGraphEditorImpl_PMS::OnAlignCenter()
{
	const FScopedTransaction Transaction(FGraphEditorCommands::Get().AlignNodesCenter->GetLabel());

	FAlignmentHelper_PMS Helper(SharedThis(this), Orient_Horizontal, EAlignType_PMS::Middle);
	Helper.Align();
}

void SGraphEditorImpl_PMS::OnAlignRight()
{
	const FScopedTransaction Transaction(FGraphEditorCommands::Get().AlignNodesRight->GetLabel());

	FAlignmentHelper_PMS Helper(SharedThis(this), Orient_Horizontal, EAlignType_PMS::Maximum);
	Helper.Align();
}

void SGraphEditorImpl_PMS::OnStraightenConnections()
{
	StraightenConnections();
}

/** Distribute the specified array of node data evenly */
void DistributeNodes(TArray<FAlignmentData_PMS>& InData, bool bIsHorizontal)
{
	// Sort the data
	InData.Sort([](const FAlignmentData_PMS& A, const FAlignmentData_PMS& B) {
	...
	if (InData.Num() > 1)
	{
		...
		if (Graph)
		{
			...
			// similar to FAlignmentHelper_PMS::Align(), first try using GraphSchema to move the nodes if applicable
			if (Schema)
			{
				for (int32 Index = 1; Index < InData.Num() - 1; ++Index)
				{
					FAlignmentData_PMS& Entry = InData[Index];
...
void SGraphEditorImpl_PMS::OnDistributeNodesH()
{
	TArray<FAlignmentData_PMS> AlignData;
	for (UObject* It : GetSelectedNodes())
	{
		if (UEdGraphNode* Node = Cast<UEdGraphNode>(It))
		{
			AlignData.Add(FAlignmentData_PMS(Node, Node->NodePosX, GetNodeSize(*this, Node).X));
		}
	}
...
void SGraphEditorImpl_PMS::OnDistributeNodesV()
{
	TArray<FAlignmentData_PMS> AlignData;
	for (UObject* It : GetSelectedNodes())
	{
		if (UEdGraphNode* Node = Cast<UEdGraphNode>(It))
		{
			AlignData.Add(FAlignmentData_PMS(Node, Node->NodePosY, GetNodeSize(*this, Node).Y));
		}
	}
...
int32 SGraphEditorImpl_PMS::GetNumberOfSelectedNodes() const
{
	return GetSelectedNodes().Num();
}

UEdGraphNode* SGraphEditorImpl_PMS::GetSingleSelectedNode() const
{
	const FGraphPanelSelectionSet SelectedNodes = GetSelectedNodes();
	return (SelectedNodes.Num() == 1) ? Cast<UEdGraphNode>(*SelectedNodes.CreateConstIterator()) : nullptr;
}

SGraphPanel* SGraphEditorImpl_PMS::GetGraphPanel() const
{
