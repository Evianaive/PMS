...
class FPMSEdGraphCommands : public TCommands<FPMSEdGraphCommands>
{

public:
        FPMSEdGraphCommands() : TCommands<FPMSEdGraphCommands>
        (
                "NodeGraphAssistant",
                NSLOCTEXT("NodeGraphAssistant", "NodeGraphAssistantCommand", "Node graph assistant command"),
                NAME_None,
                FPMSEditorStyle::Get().GetStyleSetName()
        )
        {}

        // TSharedPtr< FUICommandInfo > RearrangeNode;
        // TSharedPtr< FUICommandInfo > BypassNodes;
        TSharedPtr< FUICommandInfo > DownStreamNodesFollowMove; 
        TSharedPtr< FUICommandInfo > UpStreamNodesFollowMove;
...
inline void FPMSEdGraphCommands::RegisterCommands()
{
        ...
        UI_COMMAND(DownStreamNodesFollowMove, "Select downstream nodes", "Select all nodes connected to selected nodes' inputs", EUserInterfaceActionType::Button, FInputChord(EKeys::A, false, false, true, false));
        UI_COMMAND(UpStreamNodesFollowMove, "Select upstream nodes", "Select all nodes connected to selected nodes' outputs", EUserInterfaceActionType::Button, FInputChord(EKeys::D, false, false, true, false));
