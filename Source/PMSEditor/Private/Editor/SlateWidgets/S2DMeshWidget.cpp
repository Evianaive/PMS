...
uint32 S2DMeshWidget::AddMesh(TArray<FClipSMTriangle> InMeshData)
{	
	FRenderData& NewRenderData = RenderData[RenderData.Add(FRenderData())];
	NewRenderData.Brush = MakeShareable(new FSlateColorBrush(FLinearColor(1,1,1)));
...
uint32 S2DMeshWidget::AddMeshWithInstancing(TArray<FClipSMTriangle>& InMeshData, int32 InitialBufferSize)
{
...
void S2DMeshWidget::EnableInstancing(uint32 MeshId, int32 InitialSize)
{
...
void S2DMeshWidget::UpdatePerInstanceBuffer(uint32 MeshId, FSlateInstanceBufferData& Data)
{
